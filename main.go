...
func main() {
	...
	workers := flag.Int("workers", 1000, "Maximum number of workers used to make requests, defaults to 100.")
	flag.Parse()
	...
	getTargets(flag.Args(), cf.Submit)
	cf.Pool.StopAndWait()
}
...
func targetsFromString(s []byte, submit func(req *http.Request)) []string {
	reg, err := regexp.Compile(`([0-9]{1,3}\.){3}[0-9]{1,3}(?:/\d\d?)?`)
	if err != nil {
		panic("unable to compile regex")
	}

	resp := make([]string, 0, 10)
	for _, m := range reg.FindAll(s, math.MaxInt) {
