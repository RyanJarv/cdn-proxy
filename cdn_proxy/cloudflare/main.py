...
class CloudFlare:
    ...
    def create(self, target):
        ...
        yield 'CloudFront Proxy {} -- Creating records for {}.{}'.format(target, 'cdn-bypass-{}'.format(target), self.zone_name)
    ...
    def delete(self, target):
        yield 'CloudFront Proxy -- Retrieving records for {}.{}'.format(target, 'cdn-bypass-{}'.format(target), self.zone_name)
        dns_records = self.cf.zones.dns_records.get(
            self.zone_id,
            params={
                'name': 'cdn-bypass-{}.{}'.format(re.sub(r'\.', '-', target), self.zone_name),
            }
        )
    ...
    def list(self):
        ...
        while page == 1 or subdomains:
            subdomains = self.cf.zones.dns_records.get(self.zone_id, params={'per_page': 100, 'page': page})
