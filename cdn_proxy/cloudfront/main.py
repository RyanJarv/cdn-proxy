...
def get_function_with_tag(sess, region, key, value):
    lamb = sess.client("lambda", region_name=region)
    paginator = lamb.get_paginator('list_functions')

    for page in paginator.paginate():
        for func in page['Functions']:
            resp = lamb.list_tags(Resource=func['FunctionArn'])
            if resp['Tags'].get(key) == value:
                return func['FunctionName'], func['FunctionArn']
    return '', ''


class CloudFront:
    def __init__(self, sess: boto3.Session):
        self.sess = sess
    ...
    def create_lambda_role(self):
        ...
        iam = self.sess.client("iam", region_name=self.region)
    ...
    def create_function(self, role_arn):
        ...
        # Lambda@Edge functions need to be created in the us-east-1 region.
        lamb = self.sess.client("lambda", region_name='us-east-1')

        self.lambda_function_name, self.lambda_function_arn = get_function_with_tag(self.sess, 'us-east-1', 'cdn-proxy',
                                                                                    'request-lambda')

        if self.lambda_function_arn:
            zip.seek(0)
            resp = lamb.update_function_code(FunctionName=self.lambda_function_arn, ZipFile=zip.read(), Publish=False)
        else:
            self.lambda_function_name = 'cdn-proxy-request-' + ''.join(choice(string.digits) for _ in range(4))
        ...
        waiter = lamb.get_waiter('function_updated')
        waiter.wait(FunctionName=self.lambda_function_name)
        resp = lamb.publish_version(FunctionName=self.lambda_function_name)
        self.lambda_arn = resp["FunctionArn"]

    def delete_function(self):
        lamb = self.sess.client("lambda", region_name=self.region)
    ...
    def create_distribution(self, lambda_arn):
        ...
        client = self.sess.client("cloudfront", region_name=self.region)
    ...
    def get_lambda_versions(self):
        lamb = self.sess.client("lambda", region_name='us-east-1') # Lambda@Edge requires region us-east-1
